...
The Advanced App Priority section is functional but intentionally narrow. It lists running user-owned processes with PID, process name, nice value, CPU %, memory %, and owner, then lets the user select exactly one target process. App Priority is manual-only for now: **Apply Priority Boost Now** is the only way to boost a selected process, and automatic boosting during Performance Mode ON is not implemented. When the user explicitly chooses **Apply Priority Boost Now**, CatalinaPerformance calls `scripts/app_priority_apply.sh` as the logged-in GUI user, validates the selected process owner and expected identity immediately before boosting, saves the selected process's original nice value and identity under `~/Library/Application Support/CatalinaPerformance/app_priority/`, logs the action, and applies a conservative nice value of `-5` by default. Administrator authorization is requested only for the narrow priority-changing `renice` command when macOS requires it, and the app refuses to boost if it cannot save restore state first or if the selected process identity changed since the process list was refreshed. The selected PID is never trusted by itself: the GUI restores saved selections only when PID, owner, command, and available start/full-command identity still match, and the apply script re-checks identity again after any administrator authorization delay immediately before `renice`.
