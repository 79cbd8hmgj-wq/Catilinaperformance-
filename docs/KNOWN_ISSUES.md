...
- The process list can become stale. If App Priority rejects an apply because identity changed, or clears a saved selection after PID reuse, refresh the running-process list and select the target again. Administrator prompts can introduce a race window, so identity is checked again after authorization immediately before `renice`.
